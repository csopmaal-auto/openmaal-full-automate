...
sheet = client.open("OpenMaal_Full_Feed_Master").sheet1
...
for sku, entry in found.items():
    ...
    # still-queued row each hour (1,396 rows on this store by 2026-08-06).
    # That false Failed status then reopened generate_xml's create
    # fallback, so syncs re-submitted those products run after run (OnBuy
    # deduplicated by uid and returned the same OPC - no duplicate
